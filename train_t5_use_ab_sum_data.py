...
train_df = pd.read_csv('data/train_sum.csv')
test_df = pd.read_csv('data/test_text_sum.csv')
validation_df = pd.read_csv('data/validation_sum.csv')
...
def check_dataset_length():
    ...
    avg_sentences_tr = train_df['text_sum'].apply(count_sentences).mean()
    avg_sentences_vl = validation_df['text_sum'].apply(count_sentences).mean()
    print("number of sentence in train data = ",avg_sentences_tr )
    print("number of sentence in validation data = ",avg_sentences_vl )

    avg_text_length_tr = train_df['text_sum'].apply(lambda x: len(str(x).split())).mean()
    ...
    avg_text_length_vl = validation_df['text_sum'].apply(lambda x: len(str(x).split())).mean()
...
def plot_nb_word(df):
    ...
    df['num_words'] = df['text_sum'].apply(count_words)
...
train_df['extract_text'] = train_df.apply(lambda row: extractive_summary(row['text_sum']), axis=1)
validation_df['extract_text'] = validation_df.apply(lambda row: extractive_summary(row['text_sum']), axis=1)
